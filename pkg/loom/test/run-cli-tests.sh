...
test -f "$LOOM_REPO/loom/0.1.0/src/loom/command/link.lisple"
...
"$LISPLE" "$LOOM_PACKAGE" link "$LOOM_PACKAGE" --repo "$LOOM_LINK_REPO"
test -L "$LOOM_LINK_REPO/loom/0.1.0"
test "$("$LISPLE" "$LOOM_PACKAGE" list --repo "$LOOM_LINK_REPO")" = "loom@0.1.0"
"$LISPLE" "$LOOM_PACKAGE" uninstall loom@0.1.0 --repo "$LOOM_LINK_REPO"
