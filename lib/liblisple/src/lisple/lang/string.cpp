...
namespace Lisple
{
  namespace
  {
    ...
    std::pair<long, long> word_range(const sptr_val& value)
    {
      ...
      if (range.size() != 2)
      {
        throw LispleException("capitalize range must contain exactly two numbers.");
      }
      ...
      if (start < 0 || end < 0)
      {
        throw LispleException("capitalize range indexes must be non-negative.");
      }
      if (end < start)
      {
        throw LispleException(
          "capitalize range end must be greater than or equal to start.");
      }
  ...
  EXEC_BODY(CapitalizeFunction, exec_capitalize_count)
  {
    ...
    if (count < 0)
    {
      throw LispleException("capitalize word count must be non-negative.");
    }
