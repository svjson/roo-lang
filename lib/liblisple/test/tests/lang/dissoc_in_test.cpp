...
using DissocInFunction = LispleTest::RuntimeTestFixture;
...
TEST_F(DissocInFunction, throws_on_non_sequence_path)
{
  ...
  EXPECT_THAT(
    ...
    ThrowsMessage<Lisple::TypeError>(HasSubstr("Path for dissoc-in must be a sequence")));
}
...
TEST_F(DissocInFunction, throws_when_non_nil_scalar_blocks_path)
{
  ...
  EXPECT_THAT(
    ...
    ThrowsMessage<Lisple::TypeError>(HasSubstr("dissoc-in cannot traverse through 1")));
}
