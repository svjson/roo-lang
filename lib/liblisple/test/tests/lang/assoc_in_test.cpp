...
using AssocInFunction = LispleTest::RuntimeTestFixture;
...
TEST_F(AssocInFunction, throws_on_incomplete_path_value_chain)
{
  ...
  // When/Then
  EXPECT_THAT([this]() { runtime.eval("(assoc-in my-map [:b] 10 [:c])"); },
...
TEST_F(AssocInFunction, throws_on_non_sequence_path)
{
  ...
  // When/Then
  EXPECT_THAT([this]() { runtime.eval("(assoc-in my-map :b 10)"); },
...
TEST_F(AssocInFunction, throws_when_non_nil_scalar_blocks_path)
{
  ...
  // When/Then
  EXPECT_THAT([this]() { runtime.eval("(assoc-in my-map [:a :b] 10)"); },
              ThrowsMessage<Lisple::TypeError>(
