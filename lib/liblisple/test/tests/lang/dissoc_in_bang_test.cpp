...
using DissocInBangFunction = LispleTest::RuntimeTestFixture;
...
TEST_F(DissocInBangFunction, removal_of_key_returns_value_and_mutates_map)
{
  ...
  // Then
  EXPECT_EQ(*result, *Lisple::Value::number(10));
...
TEST_F(DissocInBangFunction, throws_when_non_nil_scalar_blocks_path)
{
  ...
  EXPECT_THAT(
    ...
    ThrowsMessage<Lisple::TypeError>(HasSubstr("dissoc-in! cannot remove property from 1")));
}
