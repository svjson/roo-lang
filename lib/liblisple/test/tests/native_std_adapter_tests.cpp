...
TEST_F(NativeStdVectorAdapter_int, get_set_children_and_count)
{
  ...
  Lisple::sptr_val adapter = Lisple::NativeStdVectorAdapter<int>::make_ref(values);

  EXPECT_EQ(Lisple::count(*adapter), 3);
  EXPECT_EQ(*Lisple::get_child(*adapter, 1), *Lisple::Value::number(2));

  Lisple::sptr_val updated = Lisple::Value::number(8);
  Lisple::Dict::set_property(adapter, Lisple::Value::number(1), updated);

  EXPECT_EQ(values.at(1), 8);
  EXPECT_EQ(*Lisple::get_child(*adapter, 1), *Lisple::Value::number(8));
...
TEST_F(NativeStdMapAdapter_int_string, get_set_keys_and_count)
{
  ...
  Lisple::sptr_val adapter = Lisple::NativeStdMapAdapter<int, std::string>::make_ref(values);

  EXPECT_EQ(Lisple::count(*adapter), 2);
  EXPECT_EQ(*Lisple::Dict::get_property(adapter, Lisple::Value::number(1)),
            *Lisple::Value::string("one"));

  Lisple::sptr_val updated = Lisple::Value::string("three");
  Lisple::Dict::set_property(adapter, Lisple::Value::number(3), updated);

  EXPECT_EQ(values.at(3), "three");
  EXPECT_EQ(*Lisple::Dict::get_property(adapter, Lisple::Value::number(3)),
            *Lisple::Value::string("three"));
...
TEST_F(NativeStdMapAdapter_int_string, script_usage)
{
  ...
  EXPECT_EQ(runtime.eval("my-map")->type, Lisple::Value::Type::NATIVE_OBJECT);
  EXPECT_EQ(*runtime.eval("(str my-map)"),
            *Lisple::Value::string(R"({1 "one" 2 "two" 3 "three"})"));

  EXPECT_EQ(*runtime.eval("(get my-map 1)"), *Lisple::Value::string("one"));
  EXPECT_EQ(*runtime.eval("(get my-map 2)"), *Lisple::Value::string("two"));
  EXPECT_EQ(*runtime.eval("(get my-map 3)"), *Lisple::Value::string("three"));
  EXPECT_EQ(*runtime.eval("(get my-map 4)"), *Lisple::Constant::NIL);
  EXPECT_EQ(*runtime.eval("(get my-map \"SEMPRINI!\")"), *Lisple::Constant::NIL);

  EXPECT_EQ(*runtime.eval("(count my-map)"), *Lisple::Value::number(3));
  runtime.eval(R"((def updated-map (assoc my-map 4 "four")))");
  EXPECT_EQ(*runtime.eval("(count updated-map)"), *Lisple::Value::number(4));
  EXPECT_EQ(*runtime.eval("(count my-map)"), *Lisple::Value::number(3));

  runtime.eval(R"((assoc! my-map 8 "eight"))");
  EXPECT_EQ(*runtime.eval("(count updated-map)"), *Lisple::Value::number(4));
  EXPECT_EQ(*runtime.eval("(count my-map)"), *Lisple::Value::number(4));
  EXPECT_EQ(*runtime.eval("(get my-map 8)"), *Lisple::Value::string("eight"));
}
...
TEST_F(NativeStdMapAdapter_uint8_short, get_set_numeric_map)
{
  ...
  Lisple::sptr_val adapter = Lisple::NativeStdMapAdapter<uint8_t, short>::make_ref(values);

  EXPECT_EQ(*Lisple::Dict::get_property(adapter, Lisple::Value::number(2)),
            *Lisple::Value::number(16));

  Lisple::sptr_val updated = Lisple::Value::number(24);
  Lisple::Dict::set_property(adapter, Lisple::Value::number(3), updated);
