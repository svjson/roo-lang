...
## Pure Roo Dependencies

Dependencies can be resolved from local paths or from the local Roo package
repository.
...
By default, Roo file namespace loading maps dots to directories:

```text
mylib.stuff.core -> mylib/stuff/core.roo
```
...
Some packages need pure Roo namespaces to be loaded eagerly because those
...
A package that combines Roo source and a native library can use this layout:
...
The manifest points Roo at `src` and points native loading at `native`:
...
The returned `RooNativePackageV1` describes the library:
...
The `load` callback receives a `RooNativeHostV1`. Register namespaces through
the host:
...
The `roo` binary uses this to run dependency-provided tools. `roo proof`
...
The `roo` binary uses this layer for package directories.
...
- `pkg/proof` is the main package with Roo source and a native library.
- `lib/libroo-package/test/tests/native_test_package.cpp` is a minimal native
  library used by package loader tests.
- `lib/libroo-package/test/tests/manifest_test.cpp` contains focused examples
