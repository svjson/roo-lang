#include "vectorgfx_native_cachedprops_adapters.h"
...
{
  const Roo::sptr_val X = Roo::Value::keyword("x");
  const Roo::sptr_val Y = Roo::Value::keyword("y");

  NATIVE_ADAPTER_IMPL(PointAdapter, Point, &POINT, (x), (y));

  NOBJ_CACHED_PROP_GET__FIELD(PointAdapter, x);
  NOBJ_CACHED_PROP_GET__FIELD(PointAdapter, y);
  ...
  {
    if (args[0]->type != Roo::Value::Type::MAP ||
        Roo::Dict::get_property(args[0], X)->type != Roo::Value::Type::NUMBER ||
        Roo::Dict::get_property(args[0], Y)->type != Roo::Value::Type::NUMBER)
      throw Roo::RooException("Invalid input");

    return PointAdapter::make_unique(
      std::get<const Roo::Value::Number>(Roo::Dict::get_property(args[0], X)->value)
        .get_float(),
      std::get<const Roo::Value::Number>(Roo::Dict::get_property(args[0], Y)->value)
        .get_float());
  }

  /** PointRotateFunction - pixils.point/rotate */
  FUNC_IMPL(PointRotateFunction,
            SIG((FN_ARGS((&POINT), (&Roo::Type::NUMBER)),
  ...
  EXEC_BODY(PointRotateFunction, exec_rotate_point)
  {
    ...
    float amount = std::get<const Roo::Value::Number>(args[1]->value).get_float();
