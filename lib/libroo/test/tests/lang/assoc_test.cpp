...
TEST_F(AssocFunction, treats_nil_as_an_empty_map)
{
  runtime.eval("(def target nil)");

  EXPECT_EQ(*runtime.eval("(assoc target :a 1 :b 2)"), *runtime.eval("{:a 1 :b 2}"));
  EXPECT_EQ(*runtime.lookup("target"), *Roo::Constant::NIL);
}

TEST_F(AssocFunction, treats_nil_as_an_empty_vector_when_all_keys_are_integers)
{
  runtime.eval("(def target nil)");
...
TEST_F(AssocFunction, treats_nil_as_an_empty_map_when_any_key_is_not_an_integer)
{
  EXPECT_EQ(*runtime.eval("(assoc nil 0 :zero :name \"value\")"),
            *runtime.eval("{0 :zero :name \"value\"}"));
}

TEST_F(AssocFunction, requires_integer_keys_to_infer_an_empty_vector)
{
