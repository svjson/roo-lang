...
namespace Roo::AST
{
  ...
  std::shared_ptr<Seq> Seq::new_sequence(Form type, size_t reserved_size)
  {
    ...
    default:
      throw RooException("Type is not a sequence");
    }
  }
  ...
  RuntimeValueWrapper::RuntimeValueWrapper(const sptr_val& val)
    : ASTNode(Form::NIL)
    , val(val)
    , delegate(NIL)
  {
  ...
  void RuntimeValueWrapper::set_property(const ASTNode& key, sptr_ast_node& value)
  {
    ...
    case Form::KEYWORD:
    {
      sptr_val v = to_rt_value(value);
      Dict::set_property(val, Roo::Value::keyword(Value<std::string>::value_of(key)), v);
      break;
    }
    default:
  ...
  void RuntimeValueWrapper::set_property(const sptr_ast_node& key,
  ...
  {
    if (val->type == Roo::Value::Type::MAP && key.get_type() == Form::KEYWORD)
    {
      auto entry = Dict::map_entry(std::get<sptr_val_v>(val->value),
                                   *Roo::Value::keyword(Value<std::string>::value_of(key)));
      if (entry.second)
      {
    ...
    if (val->type == Roo::Value::Type::OBJECT)
    {
    }
  ...
  bool RuntimeValueWrapper::has_key(const ASTNode& key) const
  {
    ...
    // }
    return Roo::Dict::contains_key(*this->val, Roo::str_val(key));
  }

  sptr_ast_node_v& RuntimeValueWrapper::get_children()
  {
    sptr_val_v vc = Roo::get_children(*val);
    child_memo.clear();
    for (auto& c : vc)
    {
  ...
  {
    return Roo::count(*val);
  }

  std::shared_ptr<ASTNode> RuntimeValueWrapper::make(const sptr_val& value)
  {
