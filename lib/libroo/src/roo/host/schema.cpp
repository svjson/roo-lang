...
namespace Roo
{
  ...
  {
    std::unordered_set<std::string> map_keys = Roo::Dict::map_string_keys(map);
    ...
    for (auto& [k, type] : required)
    {
      if (map_keys.count(k))
      {
        sptr_val val = Roo::Dict::get_property(map, k);
        if (!type->is_type_of(*val) && val->type != Value::Type::NIL)
        {
          ...
          {
            throw Roo::TypeError("Invalid type of value " + val->to_string() +
      ...
      {
        throw Roo::TypeError("AST::Map " + map.to_string() + " is missing required key :" +
                             k + " of type " + type->to_string() + ".");
      }
    }
  ...
  sptr_val MapSchema::Inspector::_get_value(const std::string& key, Value::Type type) const
  {
    ...
    if (value->type != type)
    {
      throw Roo::TypeError("Incorrect type of value for :" + key + " in " +
                           source.to_string());
    }
