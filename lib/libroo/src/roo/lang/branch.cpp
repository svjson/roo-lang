...
namespace Roo
{
  ...
  SFORM_LOWER_IMPL(CaseForm)
  {
    ...
    {
      throw RooException("case: Unmatched case pairs: " + ast_node->to_string());
    }
  ...
  SFORM_LOWER_IMPL(CondForm)
  {
    ...
    {
      throw RooException("cond: Unmatched condition pairs: " + ast_node->to_string());
    }
  ...
  SFORM_LOWER_IMPL(IfForm)
  {
    ...
    {
      throw RooException("if: Invalid number of branches: " +
  ...
  SFORM_LOWER_IMPL(WhenForm)
  {
    ...
    if (elements.size() < 2)
    {
      throw RooException("when: No condition given: " + ast_node->to_string());
    }
