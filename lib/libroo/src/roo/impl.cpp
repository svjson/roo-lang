...
namespace Roo
{
  ...
  short short_val(const AST::ASTNode& obj)
  {
    ...
    throw RooException("Cannot convert " + obj.to_string() + " to short");
  }
  ...
  unsigned short ushort_val(const AST::ASTNode& obj)
  {
    if (Type::NUMBER.is_type_of(obj))
    {
      if (auto* wrapper = dynamic_cast<const AST::RuntimeValueWrapper*>(&obj))
      {
        return std::get<const Roo::Value::Number>(wrapper->val->value).get_int();
      }
  ...
  unsigned int uint_val(const AST::ASTNode& obj)
  {
    ...
    throw RooException("Cannot convert " + obj.to_string() + " to unsigned int");
  }
  ...
  int int_val(const AST::ASTNode& obj)
  {
    ...
    throw RooException("Cannot convert " + obj.to_string() + " to int");
  }
  ...
  uint8_t uint8_val(const AST::ASTNode& obj)
  {
    ...
    throw RooException("Cannot convert " + obj.to_string() + " to uint8_t");
  }
