...
namespace Lisple::Package
{
  namespace
  {
    ...
    std::string package_error(const LispleNativePackageV1* package)
    {
    ...
    struct LoadedLibrary
    {
      ...
      const LispleNativePackageV1* package = nullptr;

      LoadedLibrary() = default;

      LoadedLibrary(LibraryHandle handle, const LispleNativePackageV1* package)
        : handle(handle)
        , package(package)
      {
      }
  ...
  LoadedNativePackages load_native_libraries(Runtime& runtime, const LoadPlan& plan)
  {
    ...
    for (const auto& library : plan.native_libraries)
    {
      ...
      auto* symbol = find_symbol(handle, LISPLE_NATIVE_ABI_SYMBOL);
      if (!symbol)
      {
        throw LispleException("Native package library '" + path +
                              "' does not export " + LISPLE_NATIVE_ABI_SYMBOL +
                              ": " + platform_error());
      }

      auto package_fn = reinterpret_cast<LispleNativePackageV1Fn>(symbol);
      const LispleNativePackageV1* package = package_fn();
      if (!package)
      {
        throw LispleException("Native package library '" + path +
                              "' returned a null package descriptor.");
      }
      if (package->abi_version != LISPLE_NATIVE_ABI_VERSION ||
          package->struct_size < sizeof(LispleNativePackageV1))
      {
        throw LispleException("Native package library '" + path +
                              "' uses an incompatible native ABI.");
      }
      if (!package->lisple_abi ||
          std::string(package->lisple_abi) != LISPLE_NATIVE_CXX_ABI)
      {
        throw LispleException("Native package library '" + path +
                              "' targets an incompatible Lisple ABI.");
      }
      ...
      LispleNativeHostV1 host{
        LISPLE_NATIVE_ABI_VERSION,
        sizeof(LispleNativeHostV1),
        &context,
        register_namespace,
        note,
      };
