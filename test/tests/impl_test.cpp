...
{
  EXPECT_EQ(Lisple::str_val(Lisple::Keyword("some-key")), "some-key");
  EXPECT_EQ(Lisple::str_val(Lisple::Symbol("bird")), "bird");
  EXPECT_EQ(Lisple::str_val(Lisple::QuotedSymbol("ARROW")), "ARROW");
...
TEST_F(subst_sexp_lmnt, substitute_first_element_of_vector)
{
  // Given
  ;
  Lisple::Vector vector = Lisple::to_AST(*runtime.eval("[1 2 3 4]"))->as<Lisple::Vector>();

  // When
  auto result = Lisple::subst_sexp_lmnt(vector, 0, std::make_shared<Lisple::String>("one"));
