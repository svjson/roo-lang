...
{
  // Given
  std::vector<std::string> string_v { "The",
                                      "Quick",
                                      "Brown",
                                      "Fox",
                                      "Ate",
...
TEST(StdVectorAdapter_string, append)
{
  ...
  // Then
  EXPECT_THAT(string_v, ElementsAre("The",
                                    "Quick",
                                    "Brown",
                                    "Fox",
                                    "Ate",
...
{
  // Given
  std::vector<Tests::Vehicle> obj_v { Tests::Vehicle("Dreamy Boom-Boom", 4),
...
TEST(StdVectorAdapter_Vehicle, get_children)
{
  ...
  EXPECT_EQ(*adapter.get_children()[0],
            *Tests::VehicleAdapter::make<Tests::Vehicle>("Dreamy Boom-Boom", 4));
  EXPECT_EQ(*adapter.get_children()[1],
            *Tests::VehicleAdapter::make<Tests::Vehicle>("Flying Saucer", 12));
  EXPECT_EQ(*adapter.get_children()[2],
            *Tests::VehicleAdapter::make<Tests::Vehicle>("Something Fast", 2));
}
...
TEST(StdVectorAdapter_Vehicle, to_string)
{
  ...
  EXPECT_EQ(adapter.to_string(),
            "[{:model-name \"Dreamy Boom-Boom\" :seats 4} {:model-name \"Flying Saucer\" :seats 12} {:model-name \"Something Fast\" :seats 2}]");
}
...
TEST(StdVectorAdapter_Vehicle, append)
{
  ...
  // Then
  EXPECT_THAT(obj_v, ElementsAre(Tests::Vehicle("Dreamy Boom-Boom", 4),
                                 Tests::Vehicle("Flying Saucer", 12),
                                 Tests::Vehicle("Something Fast", 2),
                                 Tests::Vehicle("Aniara", 40000)));
  ...
  EXPECT_EQ(*adapter.get_children()[3],
            *Tests::VehicleAdapter::make<Tests::Vehicle>("Aniara", 40000));
}
