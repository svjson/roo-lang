---
layout: reference
title: lookup.source.native.parse.comment/identity
---
...
<option value="../../../{{ publication.identity }}/namespaces/lookup.source.native.parse.comment/identity.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
