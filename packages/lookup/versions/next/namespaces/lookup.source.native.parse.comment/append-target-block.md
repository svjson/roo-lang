---
layout: reference
title: lookup.source.native.parse.comment/append-target-block
---
...
<option value="../../../{{ publication.identity }}/namespaces/lookup.source.native.parse.comment/append-target-block.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
