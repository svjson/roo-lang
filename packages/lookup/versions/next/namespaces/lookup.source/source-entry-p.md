...
<li><a class="active" href="source-entry-p.html">source-entry?</a></li>
...
<option value="../../../{{ publication.identity }}/namespaces/lookup.source/source-entry-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
