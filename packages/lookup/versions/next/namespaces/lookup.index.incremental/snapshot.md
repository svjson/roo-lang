---
layout: reference
title: lookup.index.incremental/snapshot
---
...
<li><a class="active" href="snapshot.html">snapshot</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">snapshot</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/lookup.index.incremental/snapshot.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(snapshot package contributions)
```
