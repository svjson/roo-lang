...
<li><a class="active" href="token.html">token</a></li>
</ol>
</nav>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">token</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/wraparoo.lex/token.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
<header class="symbol-heading">
<div class="symbol-namespace"><a href="../wraparoo.lex.html">wraparoo.lex</a>/</div>
<h1><code>token</code></h1>
</header>

Construct one lossless source token.

<hr class="signature-divider">

```roo
(token scalars start end kind status fields)
```
...
| start   | Inclusive scalar offset.                              |
| end     | Exclusive scalar offset.                              |
| kind    | Lexical token kind keyword.                           |
| status  | `:complete` or `:incomplete`.                         |
| fields  | Additional token fields, such as delimiter role data. |

### Returns:

A token retaining the exact source slice and a half-open scalar range.
