---
layout: reference
title: wraparoo.lex/number-token?
---
...
<li><a class="active" href="number-token-p.html">number-token?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">number-token?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/wraparoo.lex/number-token-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
<header class="symbol-heading">
<div class="symbol-namespace"><a href="../wraparoo.lex.html">wraparoo.lex</a>/</div>
<h1><code>number-token?</code></h1>
</header>

Return whether TEXT has the lexical shape of a Roo number.
...
| Arg  | Description      |
| ---- | ---------------- |
| text | Exact atom text. |

### Returns:

`true` for Roo's optional-minus decimal spelling, with at most one decimal
point after the required leading digits, and for lowercase `0x` hexadecimal
spelling; otherwise `false`.
