---
layout: reference
title: spool.context/update-node-at-path
---
...
<li><a class="active" href="update-node-at-path.html">update-node-at-path</a></li>
</ol>
</nav>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">update-node-at-path</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/spool.context/update-node-at-path.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
<header class="symbol-heading">
<div class="symbol-namespace"><a href="../spool.context.html">spool.context</a>/</div>
<h1><code>update-node-at-path</code></h1>
</header>
...
```roo
(update-node-at-path nodes path update-fn)
```
