---
layout: reference
title: proofread.rules.direct-conditions.use-cond-thread-first/conditional-transform?
---
...
<li><a class="active" href="conditional-transform-p.html">conditional-transform?</a></li>
</ol>
</nav>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">conditional-transform?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.direct-conditions.use-cond-thread-first/conditional-transform-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>conditional-transform?</code></h1>
</header>
...
```roo
(conditional-transform? {:keys [children] :as node})
```
