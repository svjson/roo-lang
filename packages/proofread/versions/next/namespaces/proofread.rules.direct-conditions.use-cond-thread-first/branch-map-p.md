---
layout: reference
title: proofread.rules.direct-conditions.use-cond-thread-first/branch-map?
---
...
<ol>
<li><a class="active" href="branch-map-p.html">branch-map?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">branch-map?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.direct-conditions.use-cond-thread-first/branch-map-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
