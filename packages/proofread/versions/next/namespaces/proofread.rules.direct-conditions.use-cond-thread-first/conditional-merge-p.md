---
layout: reference
title: proofread.rules.direct-conditions.use-cond-thread-first/conditional-merge?
---
...
<li><a class="active" href="conditional-merge-p.html">conditional-merge?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">conditional-merge?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.direct-conditions.use-cond-thread-first/conditional-merge-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>conditional-merge?</code></h1>
</header>
...
```roo
(conditional-merge? {:keys [children] :as node})
```
