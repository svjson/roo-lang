---
layout: reference
title: proofread.source.walk/package-load-roots
---
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.source.walk/package-load-roots.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(package-load-roots package-dir {:keys [load-roots] :as manifest})
```
