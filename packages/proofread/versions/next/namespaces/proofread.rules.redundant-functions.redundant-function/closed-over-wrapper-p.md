---
layout: reference
title: proofread.rules.redundant-functions.redundant-function/closed-over-wrapper?
---
...
<li><a class="active" href="closed-over-wrapper-p.html">closed-over-wrapper?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">closed-over-wrapper?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.redundant-functions.redundant-function/closed-over-wrapper-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>closed-over-wrapper?</code></h1>
</header>
...
```roo
(closed-over-wrapper? function-name param-names node)
```
