---
layout: reference
title: proofread.rules.redundant-functions.redundant-function/defun-body-index
---
...
<li><a class="active" href="defun-body-index.html">defun-body-index</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">defun-body-index</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.redundant-functions.redundant-function/defun-body-index.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>defun-body-index</code></h1>
</header>
...
```roo
(defun-body-index {:keys [children] :as node})
```
