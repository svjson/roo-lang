---
layout: reference
title: proofread.rules.redundant-functions.redundant-function/specialized-wrapper?
---
...
<li><a class="active" href="specialized-wrapper-p.html">specialized-wrapper?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">specialized-wrapper?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.redundant-functions.redundant-function/specialized-wrapper-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>specialized-wrapper?</code></h1>
</header>
...
```roo
(specialized-wrapper? param-names node)
```
