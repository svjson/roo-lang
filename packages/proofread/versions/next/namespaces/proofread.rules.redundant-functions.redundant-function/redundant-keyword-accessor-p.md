---
layout: reference
title: proofread.rules.redundant-functions.redundant-function/redundant-keyword-accessor?
---
...
<li><a class="active" href="redundant-keyword-accessor-p.html">redundant-keyword-accessor?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">redundant-keyword-accessor?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.redundant-functions.redundant-function/redundant-keyword-accessor-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>redundant-keyword-accessor?</code></h1>
</header>
...
```roo
(redundant-keyword-accessor? function-name param-names {:keys [children] :as node})
```
