---
layout: reference
title: proofread.rules.redundant-functions.redundant-function/closed-over-symbol?
---
...
<li><a class="active" href="closed-over-symbol-p.html">closed-over-symbol?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">closed-over-symbol?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.redundant-functions.redundant-function/closed-over-symbol-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>closed-over-symbol?</code></h1>
</header>
...
```roo
(closed-over-symbol? param-names node)
```
