---
layout: reference
title: proofread.rules.redundant-functions.redundant-function/redundant-function?
---
...
<li><a class="active" href="redundant-function-p.html">redundant-function?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">redundant-function?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.redundant-functions.redundant-function/redundant-function-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>redundant-function?</code></h1>
</header>
...
```roo
(redundant-function? {:keys [children] :as node})
```
