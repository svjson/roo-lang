---
layout: reference
title: proofread.rules.redundant-functions.redundant-function/wrapper-arguments?
---
...
<li><a class="active" href="wrapper-arguments-p.html">wrapper-arguments?</a></li>
</ol>
</nav>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">wrapper-arguments?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.redundant-functions.redundant-function/wrapper-arguments-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>wrapper-arguments?</code></h1>
</header>
...
```roo
(wrapper-arguments? children index param-names param? literal?)
```
