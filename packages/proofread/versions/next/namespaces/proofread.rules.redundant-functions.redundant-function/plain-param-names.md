---
layout: reference
title: proofread.rules.redundant-functions.redundant-function/plain-param-names
---
...
<li><a class="active" href="plain-param-names.html">plain-param-names</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">plain-param-names</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.redundant-functions.redundant-function/plain-param-names.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>plain-param-names</code></h1>
</header>
...
```roo
(plain-param-names params index names)
```
