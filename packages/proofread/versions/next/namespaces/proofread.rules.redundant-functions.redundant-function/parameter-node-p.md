---
layout: reference
title: proofread.rules.redundant-functions.redundant-function/parameter-node?
---
...
<li><a class="active" href="parameter-node-p.html">parameter-node?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">parameter-node?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.redundant-functions.redundant-function/parameter-node-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>parameter-node?</code></h1>
</header>
...
```roo
(parameter-node? param-names node)
```
