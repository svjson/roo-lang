---
layout: reference
title: proofread.rules.simple-expressions.redundant-let-rebind/cond-thread?
---
...
<li><a class="active" href="cond-thread-p.html">cond-thread?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">cond-thread?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.redundant-let-rebind/cond-thread-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(cond-thread? {:keys [children] :as node} name)
```
