---
layout: reference
title: proofread.rules.simple-expressions.redundant-let-rebind/reads-updated-key?
---
...
<li><a class="active" href="reads-updated-key-p.html">reads-updated-key?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">reads-updated-key?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.redundant-let-rebind/reads-updated-key-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>reads-updated-key?</code></h1>
</header>
...
```roo
(reads-updated-key? node target-name keys)
```
