---
layout: reference
title: proofread.rules.simple-expressions.redundant-let-rebind/update-step-key
---
...
<li><a class="active" href="update-step-key.html">update-step-key</a></li>
</ol>
</nav>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">update-step-key</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.redundant-let-rebind/update-step-key.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>update-step-key</code></h1>
</header>
...
```roo
(update-step-key {:keys [children] :as node})
```
