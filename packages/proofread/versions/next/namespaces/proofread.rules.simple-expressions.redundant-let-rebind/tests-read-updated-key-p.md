---
layout: reference
title: proofread.rules.simple-expressions.redundant-let-rebind/tests-read-updated-key?
---
...
<li><a class="active" href="tests-read-updated-key-p.html">tests-read-updated-key?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">tests-read-updated-key?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.redundant-let-rebind/tests-read-updated-key-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>tests-read-updated-key?</code></h1>
</header>
...
```roo
(tests-read-updated-key? children target-name keys index)
```
