---
layout: reference
title: proofread.rules.simple-expressions.redundant-let-rebind/cond-thread-update-keys
---
...
<li><a class="active" href="cond-thread-update-keys.html">cond-thread-update-keys</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">cond-thread-update-keys</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.redundant-let-rebind/cond-thread-update-keys.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>cond-thread-update-keys</code></h1>
</header>
...
```roo
(cond-thread-update-keys {:keys [children] :as node} name)
```
