---
layout: reference
title: proofread.rules.simple-expressions.use-string-starts-with/equality-candidate?
---
...
<li><a class="active" href="equality-candidate-p.html">equality-candidate?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">equality-candidate?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-string-starts-with/equality-candidate-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>equality-candidate?</code></h1>
</header>
...
```roo
(equality-candidate? {:keys [children] :as node})
```
