---
layout: reference
title: proofread.rules.simple-expressions.use-string-starts-with/leading-substr-call?
---
...
<li><a class="active" href="leading-substr-call-p.html">leading-substr-call?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">leading-substr-call?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-string-starts-with/leading-substr-call-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>leading-substr-call?</code></h1>
</header>
...
```roo
(leading-substr-call? {:keys [children] :as node})
```
