---
layout: reference
title: proofread.rules.simple-expressions.use-string-starts-with/case-candidate?
---
...
<li><a class="active" href="case-candidate-p.html">case-candidate?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">case-candidate?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-string-starts-with/case-candidate-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>case-candidate?</code></h1>
</header>
...
```roo
(case-candidate? {:keys [children] :as node})
```
