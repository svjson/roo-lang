---
layout: reference
title: proofread.rules.simple-expressions.redundant-map-check/guarded-lookup-branch?
---
...
<li><a class="active" href="guarded-lookup-branch-p.html">guarded-lookup-branch?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">guarded-lookup-branch?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.redundant-map-check/guarded-lookup-branch-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>guarded-lookup-branch?</code></h1>
</header>

Test whether an `if` branch only reads from a map-guarded value.

<hr class="signature-divider">

```roo
(guarded-lookup-branch? branch target-name fallback)
```
