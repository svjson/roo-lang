---
layout: reference
title: proofread.cli/application
---
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.cli/application.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
Proofread's supplied cli-trooper application declaration.
