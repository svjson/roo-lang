---
layout: reference
title: proofread.rules.simple-expressions.use-select-keys/check
---
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-select-keys/check.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
