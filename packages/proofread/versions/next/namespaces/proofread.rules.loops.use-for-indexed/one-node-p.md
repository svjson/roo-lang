---
layout: reference
title: proofread.rules.loops.use-for-indexed/one-node?
---
...
<li><a class="active" href="one-node-p.html">one-node?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">one-node?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.loops.use-for-indexed/one-node-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
