---
layout: reference
title: proofread.rules.loops.use-for-indexed/increment-final-body?
---
...
<li><a class="active" href="increment-final-body-p.html">increment-final-body?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">increment-final-body?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.loops.use-for-indexed/increment-final-body-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>increment-final-body?</code></h1>
</header>
...
```roo
(increment-final-body? {:keys [children]} index-name)
```
