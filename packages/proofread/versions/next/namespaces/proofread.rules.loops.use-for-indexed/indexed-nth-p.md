---
layout: reference
title: proofread.rules.loops.use-for-indexed/indexed-nth?
---
...
<li><a class="active" href="indexed-nth-p.html">indexed-nth?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">indexed-nth?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.loops.use-for-indexed/indexed-nth-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(indexed-nth? {:keys [children] :as node} collection index-name)
```
