---
layout: reference
title: proofread.rules.loops.use-for-indexed/counted-while?
---
...
<li><a class="active" href="counted-while-p.html">counted-while?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">counted-while?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.loops.use-for-indexed/counted-while-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(counted-while? {:keys [children] :as node} index-name)
```
