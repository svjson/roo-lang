---
layout: reference
title: proofread.rules.loops.use-for-indexed/contains-indexed-nth?
---
...
<li><a class="active" href="contains-indexed-nth-p.html">contains-indexed-nth?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">contains-indexed-nth?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.loops.use-for-indexed/contains-indexed-nth-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>contains-indexed-nth?</code></h1>
</header>
...
```roo
(contains-indexed-nth? node collection index-name)
```
