---
layout: reference
title: proofread.rules.loops.use-for-indexed/count-condition-target
---
...
<li><a class="active" href="count-condition-target.html">count-condition-target</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">count-condition-target</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.loops.use-for-indexed/count-condition-target.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>count-condition-target</code></h1>
</header>
...
```roo
(count-condition-target {:keys [children] :as node} index-name)
```
