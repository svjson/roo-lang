---
layout: reference
title: proofread.rules.loops.use-for-indexed/increment?
---
...
<li><a class="active" href="increment-p.html">increment?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">increment?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.loops.use-for-indexed/increment-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(increment? {:keys [children] :as node} index-name)
```
