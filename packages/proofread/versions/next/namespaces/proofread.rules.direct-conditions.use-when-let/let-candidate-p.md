---
layout: reference
title: proofread.rules.direct-conditions.use-when-let/let-candidate?
---
...
<li><a class="active" href="let-candidate-p.html">let-candidate?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">let-candidate?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.direct-conditions.use-when-let/let-candidate-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
