---
layout: reference
title: proofread.rules.direct-conditions.use-when-let/check
---
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.direct-conditions.use-when-let/check.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
