---
layout: reference
title: proofread.rules.simple-expressions.use-not-empty/not-empty-negation?
---
...
<li><a class="active" href="not-empty-negation-p.html">not-empty-negation?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">not-empty-negation?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-not-empty/not-empty-negation-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>not-empty-negation?</code></h1>
</header>
...
```roo
(not-empty-negation? {:keys [children] :as node})
```
