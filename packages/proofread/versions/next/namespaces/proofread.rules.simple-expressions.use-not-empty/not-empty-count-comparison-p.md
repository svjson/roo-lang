---
layout: reference
title: proofread.rules.simple-expressions.use-not-empty/not-empty-count-comparison?
---
...
<li><a class="active" href="not-empty-count-comparison-p.html">not-empty-count-comparison?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">not-empty-count-comparison?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-not-empty/not-empty-count-comparison-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>not-empty-count-comparison?</code></h1>
</header>
...
```roo
(not-empty-count-comparison? {:keys [children] :as node})
```
