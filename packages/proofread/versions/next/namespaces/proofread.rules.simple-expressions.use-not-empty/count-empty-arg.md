---
layout: reference
title: proofread.rules.simple-expressions.use-not-empty/count-empty-arg
---
...
<li><a class="active" href="count-empty-arg.html">count-empty-arg</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">count-empty-arg</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-not-empty/count-empty-arg.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>count-empty-arg</code></h1>
</header>
...
```roo
(count-empty-arg {:keys [children] :as node})
```
