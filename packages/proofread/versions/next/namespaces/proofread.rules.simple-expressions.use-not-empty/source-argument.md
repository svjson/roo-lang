---
layout: reference
title: proofread.rules.simple-expressions.use-not-empty/source-argument
---
...
<li><a class="active" href="source-argument.html">source-argument</a></li>
</ol>
</nav>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">source-argument</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-not-empty/source-argument.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>source-argument</code></h1>
</header>
...
```roo
(source-argument {:keys [children] :as node})
```
