---
layout: reference
title: proofread.rules.simple-expressions.use-head/replacement
---
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-head/replacement.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
Return the replacement for `(nth value 0)`.
