---
layout: reference
title: proofread.rules.simple-expressions.use-not-equal/replacement
---
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-not-equal/replacement.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
