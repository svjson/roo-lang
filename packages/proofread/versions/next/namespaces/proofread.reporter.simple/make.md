---
layout: reference
title: proofread.reporter.simple/make
---
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.reporter.simple/make.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
Create the default streaming proofread reporter.
...
A reporter map that prints diagnostics as each file is checked and prints the
