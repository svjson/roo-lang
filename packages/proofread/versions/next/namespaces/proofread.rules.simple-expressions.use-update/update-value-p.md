---
layout: reference
title: proofread.rules.simple-expressions.use-update/update-value?
---
...
<li><a class="active" href="update-value-p.html">update-value?</a></li>
</ol>
</nav>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">update-value?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-update/update-value-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(update-value? {:keys [children] :as node} target-name key)
```
