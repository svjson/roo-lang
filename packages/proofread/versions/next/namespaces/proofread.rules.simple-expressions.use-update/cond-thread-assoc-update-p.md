---
layout: reference
title: proofread.rules.simple-expressions.use-update/cond-thread-assoc-update?
---
...
<li><a class="active" href="cond-thread-assoc-update-p.html">cond-thread-assoc-update?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">cond-thread-assoc-update?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-update/cond-thread-assoc-update-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>cond-thread-assoc-update?</code></h1>
</header>
...
```roo
(cond-thread-assoc-update? parent {:keys [children] :as node})
```
