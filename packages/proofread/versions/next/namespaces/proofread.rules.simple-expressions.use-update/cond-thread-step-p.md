---
layout: reference
title: proofread.rules.simple-expressions.use-update/cond-thread-step?
---
...
<li><a class="active" href="cond-thread-step-p.html">cond-thread-step?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">cond-thread-step?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-update/cond-thread-step-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>cond-thread-step?</code></h1>
</header>
...
```roo
(cond-thread-step? {:keys [children] :as parent} node)
```
