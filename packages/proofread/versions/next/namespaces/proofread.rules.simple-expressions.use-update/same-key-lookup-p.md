---
layout: reference
title: proofread.rules.simple-expressions.use-update/same-key-lookup?
---
...
<li><a class="active" href="same-key-lookup-p.html">same-key-lookup?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">same-key-lookup?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-update/same-key-lookup-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>same-key-lookup?</code></h1>
</header>
...
```roo
(same-key-lookup? node target-name key)
```
