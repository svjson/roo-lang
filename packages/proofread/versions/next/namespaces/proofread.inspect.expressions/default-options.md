---
layout: reference
title: proofread.inspect.expressions/default-options
---
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.inspect.expressions/default-options.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
Default simple-expression inspection options.
