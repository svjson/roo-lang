---
layout: reference
title: proofread.rules.simple-expressions.redundant-let/default-options
---
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.redundant-let/default-options.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
Return the default redundant-let configuration.
...
A map describing calls treated as simple expressions.
