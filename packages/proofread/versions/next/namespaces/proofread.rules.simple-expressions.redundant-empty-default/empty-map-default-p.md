---
layout: reference
title: proofread.rules.simple-expressions.redundant-empty-default/empty-map-default?
---
...
<li><a class="active" href="empty-map-default-p.html">empty-map-default?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">empty-map-default?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.redundant-empty-default/empty-map-default-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>empty-map-default?</code></h1>
</header>
...
```roo
(empty-map-default? {:keys [children] :as node})
```
