---
layout: reference
title: proofread.rules.simple-expressions.use-keyword-lookup/defaulted-get-fallback?
---
...
<li><a class="active" href="defaulted-get-fallback-p.html">defaulted-get-fallback?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">defaulted-get-fallback?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-keyword-lookup/defaulted-get-fallback-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>defaulted-get-fallback?</code></h1>
</header>
...
```roo
(defaulted-get-fallback? {:keys [children] :as node})
```
