---
layout: reference
title: proofread.rules.simple-expressions.use-contains/presence-check?
---
...
<li><a class="active" href="presence-check-p.html">presence-check?</a></li>
</ol>
</nav>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">presence-check?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-contains/presence-check-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>presence-check?</code></h1>
</header>
