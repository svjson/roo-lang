---
layout: reference
title: proofread.rules.simple-expressions.use-contains/equality-membership-predicate?
---
...
<li><a class="active" href="equality-membership-predicate-p.html">equality-membership-predicate?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">equality-membership-predicate?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proofread.rules.simple-expressions.use-contains/equality-membership-predicate-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
<h1><code>equality-membership-predicate?</code></h1>
</header>
...
```roo
(equality-membership-predicate? {:keys [children] :as node})
```
