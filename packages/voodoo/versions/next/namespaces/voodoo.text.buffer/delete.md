---
layout: reference
title: voodoo.text.buffer/delete
---
...
<option value="../../../{{ publication.identity }}/namespaces/voodoo.text.buffer/delete.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(delete {:keys [before after] :as buffer})
```
