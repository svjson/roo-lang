...
<option value="../../../{{ publication.identity }}/namespaces/i18n/catalog.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(catalog default-locale translations)
```
