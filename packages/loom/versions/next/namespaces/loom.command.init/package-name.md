---
layout: reference
title: loom.command.init/package-name
---
...
<option value="../../../{{ publication.identity }}/namespaces/loom.command.init/package-name.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(package-name package-dir explicit-name)
```
