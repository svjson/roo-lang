...
<option value="../../../{{ publication.identity }}/namespaces/roo.io/slurp-bang.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
Read the complete contents of a text file as a string.
