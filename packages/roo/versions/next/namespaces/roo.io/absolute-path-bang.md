...
<ol>
<li><a class="active" href="absolute-path-bang.html">absolute-path!</a></li>
...
<option value="../../../{{ publication.identity }}/namespaces/roo.io/absolute-path-bang.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
Resolve a path to an absolute path using the active filesystem.
