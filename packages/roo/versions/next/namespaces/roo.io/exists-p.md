...
<li><a class="active" href="exists-p.html">exists?</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">exists?</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/roo.io/exists-p.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
Query whether any filesystem entry exists at a path.
