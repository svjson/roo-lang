---
layout: reference
title: moordown.render/backtick-run-length
---
...
<ol>
<li><a class="active" href="backtick-run-length.html">backtick-run-length</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">backtick-run-length</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/moordown.render/backtick-run-length.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
<header class="symbol-heading">
<div class="symbol-namespace"><a href="../moordown.render.html">moordown.render</a>/</div>
<h1><code>backtick-run-length</code></h1>
</header>

Return the backtick run length at INDEX in TEXT.

<hr class="signature-divider">

```roo
(backtick-run-length text index)
```
...
Number of consecutive backticks starting at INDEX.
