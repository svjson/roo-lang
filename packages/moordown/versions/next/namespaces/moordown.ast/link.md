...
<option value="../../../{{ publication.identity }}/namespaces/moordown.ast/link.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
Create a Markdown link inline node.
...
| Arg    | Description                     |
| ------ | ------------------------------- |
| label  | Link label text or inline node. |
