---
layout: reference
title: zoology.validation/validation-errors
---
...
<option value="../../../{{ publication.identity }}/namespaces/zoology.validation/validation-errors.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(validation-errors schema value)
```
