---
layout: reference
title: footsteps.progress/clamp
---
...
<option value="../../../{{ publication.identity }}/namespaces/footsteps.progress/clamp.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
```roo
(clamp min-value max-value value)
```
