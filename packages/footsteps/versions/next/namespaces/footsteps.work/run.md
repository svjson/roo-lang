...
<option value="../../../{{ publication.identity }}/namespaces/footsteps.work/run.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
| opts | Options map. Supports `:clock`, `:min-duration-ms`, `:target-duration-ms`, `:max-duration-ms`, and `:max-steps`. |

### Returns:

A result map like `slice`, including `:steps-run`, `:elapsed-ms`, and `:stop-reason`.
