...
<option value="../../../{{ publication.identity }}/namespaces/footsteps.work/slice.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
| opts | Options map. Supports `:clock`, `:target-duration-ms`, legacy `:min-duration-ms`, `:max-duration-ms`, and `:max-steps`. |
