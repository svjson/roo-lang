...
<option value="../../../{{ publication.identity }}/namespaces/footsteps/run.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
...
| opts  | Optional map. Supports `:clock`, `:min-duration-ms`, `:target-duration-ms`, `:max-duration-ms`, and `:max-steps`. |
| &rest |                                                                                                                   |
