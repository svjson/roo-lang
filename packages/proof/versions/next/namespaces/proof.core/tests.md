...
<option value="../../../{{ publication.identity }}/namespaces/proof.core/tests.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
