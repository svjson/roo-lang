...
<li><a class="active" href="assert.html">assert</a></li>
...
<span class="breadcrumb-separator" aria-hidden="true">/</span>
<span aria-current="page">assert</span>
</nav>
...
<option value="../../../{{ publication.identity }}/namespaces/proof.core/assert.html"{% if publication.identity == "next" %} selected{% endif %}>{{ publication.identity }}</option>
{% endfor %}
</select></label>
</div>
