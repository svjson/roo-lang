...
namespace Lisple
{
  namespace Server
  {
    ...
    {
      return properties.contains(name) &&
        std::holds_alternative<Block>(properties.at(name));
    }
    ...
    std::optional<Error> MessageBuilder::set_property(const std::string& name, const std::string& value)
    {
      if (ctx_stack.empty())
      {
        return Error { 0x00, "Messages must open with an !MSG block"};
    ...
    Response MessageBuilder::build_simple_response(const std::string& id, const std::string& body)
    {
    ...
    Response MessageBuilder::build_response_with_props(const std::string& id,
    ...
    std::variant<std::string, Error> MessageParser::read_symbol(const std::string& data, size_t* i)
    {
      std::string symbol;
      size_t begin =*i;
      size_t end = *i;
    ...
    std::variant<std::string, Error> MessageParser::read_value(const std::string& data, size_t* i)
    {
      std::string symbol;
      size_t begin =*i;
      size_t end = *i;
    ...
    std::optional<Error> MessageParser::read_begin_block(const std::string& data, size_t* i, MessageBuilder* builder)
    {
      auto symbol = read_symbol(data, i);
      if (auto* err = std::get_if<Error>(&symbol))
        return *err;
    ...
    std::optional<Error> MessageParser::read_end_block(const std::string& data, size_t* i, MessageBuilder* builder)
    {
      auto symbol = read_symbol(data, i);
      if (auto* err = std::get_if<Error>(&symbol))
        return *err;
    ...
    std::optional<Error> MessageParser::read_property(const std::string& data, size_t* i, MessageBuilder* builder)
    {
      auto symbol = read_symbol(data, i);
      if (auto* err = std::get_if<Error>(&symbol))
        return *err;
